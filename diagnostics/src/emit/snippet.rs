...
impl<'a> Snippet<'a> {
    fn source_slice(&self, span: Span) -> String {
        span.file.source[span.start.offset..span.end.offset].to_string()
    }
}
...
impl<'a> Emit for Snippet<'a> {
    fn emit(&self, e: &mut impl WriteColor) -> io::Result<()> {
        let first_file = super::file::FileName {
            name: &self.labels[0].span.unwrap().file.name,
            ...
            if span.start.line == span.end.line {
            ...
            } else {
                {
                    ...
                    line.annotations.push(source::Annotation {
                        ...
                        kind,
                        msg: None,
                        meta: (span.start.col, span.file.source.lines().nth(span.start.line).unwrap().len()),
                    });
                }
                
                {
                    ...
                    line.annotations.insert(0, source::Annotation {
                        color: label_style.clone(),
